# Token usage tracking — CatAdventure
...
| Scripted agent sessions (API) | 17 |
| Models | deepseek-v4-flash, deepseek-v4-pro |
| Messages | 953 |
| API calls | 804 |
| Input tokens | 1 112 571 |
| Output tokens | 480 273 |
| Of which reasoning | 287 354 |
| Cache read (cache_read) | 141 952 256 |
| Cache write (cache_write) | 0 |
| **Total (input + output)** | **1 592 844** |
| Estimated cost | ≈ 1.347 USD |
...
sqlite3 ~/.hermes/state.db "SELECT id, started_at, model,
  ...
  FROM sessions WHERE cwd LIKE '%catadventure%'
  ORDER BY started_at;"
```
